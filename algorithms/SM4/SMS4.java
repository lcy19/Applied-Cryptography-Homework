...
// 版权声明：本文为CSDN博主「pagekpang」的原创文章，遵循 CC 4.0 BY-SA 版权协议，转载请附上原文出处链接及本声明。
// 原文链接：https://blog.csdn.net/wak0408/article/details/51714636
// 部分注释添加  lcy, 2019年10月27日 
...
public class SMS4 {
    ...
    /*
    @param Input: 128 bit plaintext
    @param rk: round key, 32 * 8 bit 
    @return Output:128 bit ciphertext
    */
	public void SMS4Crypt(byte[] Input, byte[] Output, int[] rk) {
        ...
        // split Input to four parts, cause every int can store 32bits
		for (int i = 0; i < 4; i++) {
    ...
    @return rk : 32 * 8 bit
    @param CrypFlag: the flag about encryption or decryption
    */
	private void SMS4KeyExt(byte[] Key, int[] rk, int CryptFlag) {
        ...
        // logical excluive-or with system parameter FK
		x[0] ^= 0xa3b1bac6;
		x[1] ^= 0x56aa3350;
		x[2] ^= 0x677d9197;
        x[3] ^= 0xb27022dc;
	...
	@param inLen:  
	@param key: 128 bit cipher key
	@param CryptFlag: the flag about encryption or decryption
	@return out:
	*/
